...
static BOOL is_window_state_changed( LONG style, INT cmd )
{
    ...
    case SW_SHOWDEFAULT:
        if (is_visible && !(style & (WS_MINIMIZE | WS_MAXIMIZE)))
            return FALSE;
        break;
    default:
        return FALSE;
...
static BOOL show_window( HWND hwnd, INT cmd )
{
    ...
    if (!is_window_state_changed( style, cmd ))
        goto done;
